...
void Player::update(SDL_Renderer* renderer){
    ...
    if (GetAsyncKeyState('Q')){l=true;}else l=false;
    //if (GetAsyncKeyState('E')){r=true;}else r=false;
    ...
    if (kool_down_null_comm > 0) kool_down_null_comm--;
        ...
        {
            if(get_gun_type()==1){
                bullets.push_back(Bullet(x_pos,y_pos,x_mouse + (player.x_pos - (win_w / 2)),y_mouse + (player.y_pos - (win_h / 2)),12,15,pistol_bullet,2,1));
                timebullet = 200;
                Mix_PlayChannel(3, pistolet, 0);
            }
            if(get_gun_type()==2){
                bullets.push_back(Bullet(x_pos,y_pos,x_mouse + (player.x_pos - (win_w / 2)),y_mouse + (player.y_pos - (win_h / 2)),13,23,automat_bullet,3,2));
                timebullet = 50;
                Mix_PlayChannel(3, avtomat, 0);
                Mix_VolumeChunk(avtomat, 20);
            }
            if(get_gun_type()==3){
