...
void inichialization_factory()
{
    ...
    prisoner.finel_vert = true;
    rand_spawn(60,27);
    player.set_texture(tex);
    helicopter.y_pos = 500 - 3000;
    helicopter.time_polet = -1;
}
...
void init_menu(SDL_Renderer* renderer){
    ...
    texture_from_image(renderer, tex1, image, "Textures\\MainMenu\\SETTINGS.png");
    texture_from_image(renderer, tex2, image, "Textures\\MainMenu\\SETTINGS_light.png");
    settings = Button(tex1, tex2, 315, 600, 495, 90, 545, 140, 0);
    ...
    exit1 = Button(tex1, tex2, 452, 750, 230, 90, 280, 140, 200);
////////
    texture_from_image(renderer, tex1, image, "Textures\\MainMenu\\START.png");
    texture_from_image(renderer, tex2, image, "Textures\\MainMenu\\STARTLIGHT.png");
...
////////
    ef_r = Button(arrow, act_arrow, win_w/2 + 264, win_h/2 + 200, 80, 80, 80, 80, 0);
    mus_r = Button(arrow, act_arrow, win_w/2 + 264, win_h/2 - 140, 80, 80, 80, 80, 0);
////////
    ...
    l_arrow = Button(arrow, act_arrow, win_w/2 - 480, win_h/2 - 40, 80, 80, 80, 80, 0);
////////
    ef_l = Button(arrow, act_arrow, win_w/2 - 344, win_h/2 + 200, 80, 80, 80, 80, 0);
    mus_l = Button(arrow, act_arrow, win_w/2 - 344, win_h/2 - 140, 80, 80, 80, 80, 0);
////////
...
void render_els(SDL_Renderer* renderer, SDL_Texture* el, int value, int x, int y, int w, int h, int space){
    int x_pos = x, sp = space;
    for(int i = 0; i < value; i++){
        if(i == 0){x = x_pos + (w * i);}
        else{x = x_pos + (w * i) + space, space += sp;}
        render_texture(renderer, el, x, y, w, h);
    }
}

void options(SDL_Renderer* renderer, SDL_Event event){
    SDL_Texture* el;
    SDL_Texture* ef_text;
    SDL_Texture* mus_text;
    texture_from_image(renderer, ef_text, image, "Textures\\MainMenu\\effects.png");
    texture_from_image(renderer, mus_text, image, "Textures\\MainMenu\\music.png");
    texture_from_image(renderer, el, image, "Textures\\MainMenu\\sound_el.png");
    while(!quit && opt){
        SDL_RenderClear(renderer);
        SDL_PollEvent(&event);
        switch (event.type){
        case SDL_QUIT:
            ...
            break;
        case SDL_MOUSEBUTTONDOWN:
            if(ef_l.click() && ef_loud >= 16) ef_loud -= 16;
            if(ef_r.click() && ef_loud <= 112) ef_loud += 16;
            if(mus_l.click() && mus_loud >= 16) mus_loud -= 16;
            if(mus_r.click() && mus_loud <= 112) mus_loud += 16;
            break;
        case SDL_KEYDOWN:
            switch (event.key.keysym.sym) {
            case SDLK_ESCAPE:
        ...
        render_els(renderer, el, mus_loud / 16, win_w / 2 - 244, win_h / 2 - 140, 40, 80, 24);
        render_els(renderer, el, ef_loud / 16, win_w / 2 - 244, win_h / 2 + 200, 40, 80, 24);
        render_texture(renderer, ef_text, win_w / 2 - 355, win_h / 2 + 20, 710, 130);
        render_texture(renderer, mus_text, win_w / 2 - 255, win_h / 2 - 310, 510, 130);
        mus_r.update(renderer);
        mus_l.update(renderer);
        ef_r.update(renderer);
        ef_l.update(renderer);
        Mix_VolumeMusic(mus_loud);
        Mix_Volume(-1, ef_loud);
        SDL_RenderPresent(renderer);
    }
}
...
void Game(SDL_Renderer* renderer){
    ...
    SDL_Event event;
    bool main_music = false, level_menu = false;
    ...
    helicopter = Helicopter(3159,500,80*6,178*6);
    prisoner = Prisoner(1300,1300,96,96);
    //Bot_tanks.push_back(Bot_tank(900,400,33*4,32*4,1,0));
    scene = Scene(backgr);
            ...
            while(!quit && game){
                ...
                if(player.lives_bar<=0)
                {
                    ...
                    for(int i = 0; i < 1000; i++){
                        if(i == 0)Mix_VolumeMusic(128), Mix_PlayMusic(over, 0);
                        SDL_Delay(1);
